...
last_opened_file_path :string = "none"
...
def open_file(frame, file_path):
    last_opened_file_path = file_path
    active_frame(file_reader_frame.file_reader_main_frame(frame, file_path), "np")
...
#         ).pack()
    ...
    def add_book_cover_and_info(self, file_path, file_type): #workaround for sorting is creating arrays that are sorted by date/alphabetically/etc, and just following array
        ...
        file_cover_image_button = tk.Button(
            ...
            # lambda: active_frame(file_reader_frame.file_reader_main_frame(self.frame, file_path), "np")
            )
        file_cover_image_button.image = photo
        file_cover_image_button.pack()
